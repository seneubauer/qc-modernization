...
def scrape_all(qc_folder: str, anchor_search_term: str, file_extension: str, qty_limit = 0, is_random = False, workbooks = []) -> list:
	...
	filtered_contents = list(filter(lambda item:
							os.path.isfile(os.path.join(qc_folder, item))
							and item[:1].lower() != "~"
							and item[:1].lower() != "_"
	...
	files = []
	if is_random:
		files = random.sample(filtered_contents, qty_limit)
	else:
	...
	for item in files:
		...
		wb_contents = scrape_one(qc_folder, anchor_search_term, item)
		
		# extend output list if wb_contents is valid
		if wb_contents is not None:
			output.append({ "workbook": item, "worksheets": wb_contents })
	
	# return the results
	return output

# save results to a binary file
def to_binary(destination_folder: str, file_name: str, data_object: list) -> None:

	# create, populate, then close the binary file
	my_file = open(os.path.join(destination_folder, file_name), "wb")
	pickle.dump(data_object, my_file)
	my_file.close()

# save results to csv files
def to_csv(destination_folder: str, file_extension: str, data_object: list) -> None:

	# iterate through the list and save the DataFrame objects as csv files
	for wb in data_object:
		for ws in wb["worksheets"]:
			wb_name = wb["workbook"].replace(file_extension, "")
			ws_name = ws["worksheet"]
			ws_df = ws["data"]
			csv_name = f"{wb_name}_{ws_name}.csv"
			ws_df.to_csv(os.path.join(destination_folder, csv_name), index = False)

# convert binary file to multiple csv files
def binary_to_csvs(binary_folder: str, binary_name: str, destination_folder: str, file_extension: str) -> None:

	# open, read, then close the binary file
	bin_file = open(os.path.join(binary_folder, binary_name), "rb")
	file_contents = pickle.load(bin_file)
	bin_file.close()

	# save the file contents to csv files
	to_csv(destination_folder, file_extension, file_contents)